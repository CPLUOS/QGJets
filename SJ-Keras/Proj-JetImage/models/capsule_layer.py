"""
ref. https://github.com/XifengGuo/CapsNet-Keras/blob/master/capsulelayers.py
"""
...
class Mask(layers.Layer):
    ...
    capsule with max length or by an additional input mask. Except the
    max-length capsule (or specified capsule), all vectors are masked to zeros.
    ...
    For example:
        ...
        out = Mask()(x)
        # or
        # out2.shape=[8,6]. Masked with true labels y. Of course y can also be manipulated.
        out2 = Mask()([x, y])          ```
    """
    def call(self, inputs, **kwargs):
...
class CapsuleLayer(layers.Layer):
    ...
    inputs, each is a scalar, the output of the neuron from the former layer,
    and it has `out_num` output neurons. CapsuleLayer just expand the output 
    of the neuron from scalar to vector. So its
